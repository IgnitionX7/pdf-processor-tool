...
Combines figure_extractor.py and table_extractor.py into a single script
Creates an output folder named after the input PDF file
"""
...
class PDFProcessor:
    """Unified PDF processor that extracts both figures and tables"""

    def __init__(self, pdf_path: str, output_dir: Optional[str] = None):
        self.pdf_path = Path(pdf_path)

        # If no output dir specified, create folder named after PDF file
        if output_dir is None:
            # Get PDF filename without extension
            pdf_name = self.pdf_path.stem
            self.output_dir = Path(pdf_name)
        else:
            self.output_dir = Path(output_dir)
    ...
    def process(self) -> Dict:
        ...
        print("="*60)
        figure_extractor = FigureExtractor(output_dir=str(self.output_dir))
        ...
        print("="*60)
        table_extractor = TableExtractor(output_dir=str(self.output_dir))
    ...
    def generate_summary(self, results: Dict) -> str:
        """
        Generate a combined summary report of extracted figures and tables
        """
        summary_lines = []
        summary_lines.append("# PDF Extraction Summary\n\n")
        summary_lines.append(f"**Source PDF:** {results['pdf_path']}\n\n")
        summary_lines.append(f"**Output Directory:** {results['output_dir']}\n\n")
        summary_lines.append(f"**Total Figures:** {results['total_figures']}\n\n")
        summary_lines.append(f"**Total Tables:** {results['total_tables']}\n\n")
        summary_lines.append("---\n\n")

        # Figures section
        if results['figures']:
            summary_lines.append("## Extracted Figures\n\n")
            for fig in results['figures']:
                summary_lines.append(f"### Fig. {fig['fig_num']} (Page {fig['page']})\n\n")
                summary_lines.append(f"- **File:** `{fig['filename']}`\n")
                summary_lines.append(f"- **Caption:** {fig['caption']}\n\n")
                summary_lines.append(f"![Fig. {fig['fig_num']}]({fig['filename']})\n\n")
                summary_lines.append("---\n\n")

        # Tables section
        if results['tables']:
            summary_lines.append("## Extracted Tables\n\n")
            for table in results['tables']:
                summary_lines.append(f"### Table {table['table_num']} (Page {table['page']})\n\n")
                summary_lines.append(f"- **File:** `{table['filename']}`\n")
                summary_lines.append(f"- **Caption:** {table['caption']}\n\n")
                summary_lines.append(f"![Table {table['table_num']}]({table['filename']})\n\n")
                summary_lines.append("---\n\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Extract both figures and tables from PDF files',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Extract to folder named after PDF
  python pdf_figure_table_extractor.py document.pdf

  # Extract to custom folder
  python pdf_figure_table_extractor.py document.pdf --output-dir my_outputs

  # Generate summary report
  python pdf_figure_table_extractor.py document.pdf --summary
        """
    )
    parser.add_argument('pdf_path', help='Path to input PDF file')
    parser.add_argument('--output-dir',
                       help='Output directory (default: folder named after PDF file)')
    parser.add_argument('--summary', action='store_true',
                       help='Generate summary markdown file')

    args = parser.parse_args()

    # Create processor
    processor = PDFProcessor(pdf_path=args.pdf_path, output_dir=args.output_dir)
